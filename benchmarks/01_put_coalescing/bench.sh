...
for elem in "${elements_list[@]}"; do
  for grid in "${grids_list[@]}"; do
    for block in "${blocks_list[@]}"; do
      ...
      echo -n "01_put_coalescing,1,$elem,100,1,$block" >>$output_file
      nvshmrun -np 2 ./bench_01_put_coalescing $elem 100 1 $block >>$output_file
    done
  done
done
...
for elem in "${elements_list[@]}"; do
  for grid in "${grids_list[@]}"; do
    for block in "${blocks_list[@]}"; do
      ...
      echo -n "01_put_coalescing,2,$elem,100,1,$block" >>$output_file
      nvshmrun -n 2 -ppn 2 --hosts 10.0.2.11,10.0.2.12 ./bench_01_put_coalescing $elem 100 1 $block >>$output_file
    done
  done
done
