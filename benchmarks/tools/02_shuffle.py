...
data = pd.read_csv('results/02_shuffle.csv')

# this tuple size has the best results and is the most realistic
data = data[(data['tuple_size'] == 32)
            & (data['send_buffer_size_multiplier'] == 1)]
...
# Create the 3D plot with adjusted figure size and font size
...
plt.title('Shuffle Throughput', fontsize=20)
